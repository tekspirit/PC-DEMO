		...
		if (queue->step==STEP_TRANSACTION)
		{
			...
			while(transaction)
			{
				if (transaction->flag)
				{
					...
					memcpy(&transaction->deal,(uint32 *)(queue->data+1*sizeof(uint32)),sizeof(deal));
					memcpy(transaction->plain,(uint32 *)(queue->data+1*sizeof(uint32)+sizeof(deal)),KEY_LEN);
					memcpy(transaction->cipher,(uint32 *)(queue->data+1*sizeof(uint32)+sizeof(deal)+KEY_LEN),KEY_LEN);
